  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: RegisterRequest = {
        ...
        nickname: form.nickname?.trim() ? form.nickname : undefined,
      };
      ...
      setError(err.response?.data?.error || '注册失败，请检查填写信息');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen max-h-screen overflow-hidden grid lg:grid-cols-2 bg-white font-sans text-gray-900">
      <div className="relative hidden lg:flex flex-col justify-between bg-gradient-to-br from-gray-400 via-gray-500 to-gray-600 p-12 text-white overflow-hidden">
        ...
        <div className="relative z-20">
          <h2 className="text-[30px] font-extrabold tracking-tight leading-tight max-w-md">Build your workspace and start shipping together.</h2>
          <p className="mt-4 text-sm text-gray-200 max-w-sm font-medium">
            Create your account to organize boards, track progress, and collaborate with your team in one place.
          </p>
          <div className="mt-10 flex items-center gap-8 text-sm text-gray-200">
      ...
      <div className="flex items-center justify-center p-8 bg-white relative">
        <div className="w-full max-w-[420px]">
          ...
          <div className="text-center mb-10">
            <h1 className="text-3xl font-extrabold tracking-tight text-gray-900 mb-2">Create your account</h1>
            <p className="text-gray-500 text-sm font-medium">Enter your details to get started.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-700" htmlFor="username">Username</label>
              <input
                type="text"
                id="username"
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-700" htmlFor="nickname">Nickname</label>
              <input
                type="text"
                id="nickname"
                className="flex w-full rounded-full border border-gray-200 px-5 py-2 text-sm h-12 bg-white text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0d6efd]/20 focus:border-[#0d6efd] transition-all"
          ...
          <div className="text-center text-sm text-gray-500 mt-8 font-medium">
            Already have an account? <Link className="text-gray-900 font-bold hover:underline" to="/login">Sign in</Link>
          </div>
        </div>
      </div>
    </div>
