      ...
      <div className="flex h-screen bg-[#f8fafc] font-sans antialiased text-gray-800">
        ...
        <main className="flex-1 flex items-center justify-center bg-[#fcfcfc]">
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="text-gray-500 font-semibold tracking-wider text-sm animate-pulse">LOADING ANALYTICS...</div>
          </div>
        </main>
      </div>
    );
        ...
        <main className="flex-1 flex flex-col bg-[#fcfcfc]">
          ...
          <div className="flex-1 px-10 py-10">
            <div className="mx-auto flex max-w-[860px] flex-col items-center justify-center rounded-[2rem] border border-dashed border-gray-200 bg-white px-10 py-20 text-center shadow-sm">
              ...
              <h1 className="text-[30px] font-extrabold tracking-tight text-gray-900">No boards to analyze yet</h1>
              <p className="mt-3 max-w-xl text-[15px] leading-7 text-gray-500">
                Create a board or add yourself to an existing workspace board to unlock cycle time, throughput, and CFD charts.
              </p>
          ...
          <div className="mx-auto max-w-[1240px]">
            <div className="mb-10 flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
              <div>
                <div className="mb-4 flex items-center gap-2 text-[12px] font-extrabold uppercase tracking-widest text-gray-500">
                  ...
                  <span className="text-gray-900">Board Analytics</span>
                </div>
            ...
            {boardLoading || !board ? (
              <div className="rounded-[2rem] border border-gray-100 bg-white px-8 py-24 text-center text-gray-500 shadow-sm">
                Loading selected board analytics...
              </div>
            ) : (
              <>
                <div className="mb-8 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
                  <div className="rounded-[1.75rem] border border-gray-100 bg-white p-6 shadow-sm">
                    <p className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-gray-400">Completion</p>
                    <p className="mt-3 text-[34px] font-extrabold tracking-tight text-gray-900">{completionRate}%</p>
                    <p className="mt-2 text-sm font-medium text-gray-500">{completedCards} of {cards.length} cards completed</p>
                  </div>
                  <div className="rounded-[1.75rem] border border-gray-100 bg-white p-6 shadow-sm">
                    <p className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-gray-400">Active Cards</p>
                    <p className="mt-3 text-[34px] font-extrabold tracking-tight text-gray-900">{activeCards}</p>
                    <p className="mt-2 text-sm font-medium text-gray-500">Open work currently in motion across {lists.length} lists</p>
                  </div>
                  <div className="rounded-[1.75rem] border border-gray-100 bg-white p-6 shadow-sm">
                    <p className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-gray-400">Due Soon</p>
                    <p className="mt-3 text-[34px] font-extrabold tracking-tight text-gray-900">{dueSoonCards}</p>
                    <p className="mt-2 text-sm font-medium text-gray-500">Cards due within the next 7 days</p>
                  </div>
                  <div className="rounded-[1.75rem] border border-gray-100 bg-white p-6 shadow-sm">
                    <p className="text-[11px] font-extrabold uppercase tracking-[0.18em] text-gray-400">Board Shape</p>
                    <p className="mt-3 text-[34px] font-extrabold tracking-tight text-gray-900">
                      {lists.length}/{board.swimlanes?.length || 0}
                    </p>
                    <p className="mt-2 text-sm font-medium text-gray-500">Lists / Swimlanes in this board configuration</p>
                  </div>
                </div>
